...
if ($action=="add") {
	...
	if($row!=0){
		...
		$row=mysqli_num_rows($results);
		if ($row!=0) {
			$sql="INSERT INTO `wishlist` (`user_id`, `videos_id`) VALUES ($user_id,$videos_id)";
			if ($results=mysqli_query($conn,$sql)) {
				$response = array("status" => true , "msg" => "Added To Watchlist", "action"=>"add",);
			}else{
	...
	if($row!=0){
		...
		if ($row!=0) {
			...
			if ($results=mysqli_query($conn,$sql)) {
				$response = array("status" => true , "msg" => "Removed from Watchlist", "action"=>"remove");
			}else{
				$response = array("status" => false , "msg" => "Something went wrong");
			}
		}else{
